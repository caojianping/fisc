<template>
    <div>
        <div v-transfer-dom>
            <x-popup v-model="isShow" position="top" @on-hide="hidePopup">
                ...
                <x-group>
                    ...
                    <x-selector v-model="params.seeType" direction="rtl" :title="titles.seeType"
                                :options="seeTypes"></x-selector>
        ...
        <MobileTable :type="'rank'" :url="url" :params="params" :columns="columns" :callback="callback"
                     :isRefresh="isRefresh" :isFlag="true" @changeRefresh="(refresh)=>{this.isRefresh=refresh}"
...
<script>
    ...
    const analyzerCtrl = new AnalyzerController('rank', true);
    analyzerCtrl.init();
